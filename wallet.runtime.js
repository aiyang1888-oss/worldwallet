...
(function wwMigratePlainPinToHashOnce() {
  if (typeof savePinSecure !== 'function') {
    try {
      if (typeof showToast === 'function') showToast('安全模块未加载，PIN 无法迁移保存', 'error');
    } catch (_e) {}
    return;
  }
  try {
...
/* createRealWallet / restoreWallet 以 wallet.core.js（importWalletFlexible + 加密）为准 */
...
function getMnemonicStrengthDisplay() {
  var n = 12;
  // 创建流程以临时钱包词数为准；否则以 REAL_WALLET（不依赖可能被浏览器恢复的下拉框）
  var _twM = typeof wwGetTempWallet === 'function' ? wwGetTempWallet() : null;
  if (_twM && _twM.mnemonic) {
    var wct = _twM.mnemonic.trim().split(/\s+/).filter(Boolean).length;
...
/* sendTRX / sendETH / sendUSDT_TRC20 以 wallet.tx.js 为准（解锁 + 派生 + 立即清理） */
...
async function doTransfer() {
  const ta = document.getElementById('transferAddr');
  const amtInp = document.getElementById('transferAmount');
  const addr = ta ? String(ta.value || '').trim() : '';
...
function shareSuccess(fromEl) {
  ...
  const txText = txEl && txEl.textContent ? String(txEl.textContent).trim() : '';
  ...
  if (btn) {
    ...
    if (nested) {
      var oldN = nested.textContent || '';
      nested.textContent = '✅ 已复制';
      setTimeout(function() { nested.textContent = oldN; }, 1500);
    } else {
      var oldT = btn.textContent || '';
      btn.textContent = '✅ 已复制';
      setTimeout(function() { btn.textContent = oldT; }, 1500);
    }
  }
}
...
// ── 交易历史：实现见 wallet.tx.js（TRON_GRID + wwFetch429Retry）；此处勿重复定义以免覆盖 ──
...
async function startVerify() {
  ...
  if (_twRt && _twRt.mnemonic) {
    if (_twRt.words && _twRt.words.length >= 12) {
      words = _twRt.words.slice();
    } else {
      words = _twRt.mnemonic.trim().split(/\s+/).filter(Boolean);
    }
  }
  ...
  if (!words || words.length < 12) {
    ...
    try {
      var _twrRt = (typeof wwGetTempWallet === 'function' ? wwGetTempWallet() : null) || {};
      if (typeof wwSetTempWallet === 'function') wwSetTempWallet(_twrRt);
...
async function pinVerifyEnterWallet() {
  ...
  wwSetSessionPin(pin);
  try {
    if (REAL_WALLET && REAL_WALLET.hasEncrypted) {
      var sensitivePv = await decryptSensitive(pin);
...
async function _resumeWalletAfterUnlock() {
  ...
  if (pin && REAL_WALLET && REAL_WALLET.hasEncrypted) {
    try {
      var sensitiveU = await decryptSensitive(pin);
...
async function wwExportEncryptedKeyBackup() {
  var mnExport = '';
  if (typeof wwGetSessionMnemonic === 'function') mnExport = wwGetSessionMnemonic();
  if (!mnExport) {
    var _twe = typeof wwGetTempWallet === 'function' ? wwGetTempWallet() : null;
    if (_twe && _twe.mnemonic) mnExport = String(_twe.mnemonic);
  }
  if (!REAL_WALLET || !mnExport) { if (typeof showToast === 'function') showToast('请先解锁钱包后再导出（会话内需已解密助记词）', 'warning'); return; }
